
import pyautogui
import time
time.sleep(6)
pyautogui.click(430,58)
pyautogui.moveTo(280,345, duration = 0.75)
pyautogui.dragTo(480,645)
pyautogui.click(390,58)
pyautogui.moveTo(480,480)
pyautogui.dragTo(780,480)
pyautogui.moveTo(520,490)
pyautogui.dragTo(520,610)
pyautogui.moveTo(520,470)
pyautogui.dragTo(520,310)
pyautogui.moveTo(780,480)
pyautogui.dragTo(800,620)
pyautogui.moveTo(780,480)
pyautogui.dragTo(800,320)
# executed by identifying the pixel location (manually) and moving the cursor using python
